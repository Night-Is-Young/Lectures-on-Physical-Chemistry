...
\begin{derivation}
    在等温等压下,含有上述\ce{A}和\ce{B}物质的系统的Gibbs自由能的微分
    ...
    回顾化学势的定义,$\mu_{\ce{B}}-\mu_{\ce{A}}$不仅代表\ce{B}与\ce{A}在这样的系统组成下的化学势的差值,也等于在大量(该组成的)系统中反应进行$1\mol$上述反应的Gibbs自由能变.%
    这就是摩尔Gibbs自由能变的定义.
\end{derivation}
...
\end{derivation}
\begin{theorem}[5B.1.5 由$\Delta_\r H_{\m}^\ominus$和$\Delta_\r S_{\m}^\ominus$求算$\Delta_\r G_{\m}^\ominus$]
    我们有
