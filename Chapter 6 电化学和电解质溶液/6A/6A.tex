...
\noindent\tbf{\LARGE 6A 氧化还原反应与化学电池}\vspace{15pt}\\
...
\begin{hint}
    通常,盐桥琼脂以及其中含有的饱和\ce{KCl}溶液或饱和\ce{KNO3}溶液构成.%
    这两种盐的溶解度都较大,且阴阳离子迁移速率差别不大,因此可以有效减小液接电势\footnotemark.%
    此外,使用盐桥还可以防止某些离子进入阴极或阳极的溶液造成副反应.
\end{hint}\footnotetext{我们将在\tbf{6B}讨论液接(液体接界)电势的成因.现在,你只需要知道它是由两种相接溶液的离子浓度不同所致即可.}
\Part{电池的表示方法}
...
\begin{theorem}[6A.2.3 电池的书写方法]
    ...
    \begin{enumerate}[topsep=0pt,parsep=0pt,itemsep=0pt,partopsep=0pt,label=\tbf{\arabic*.},leftmargin=*]
        ...
        \item 用竖虚线\hspace{1.5pt}\dashrule{0.5}{2 1.5 2 1.5 2}\hspace{1.5pt}表示半透膜等产生液接电势的界面.
...
\begin{tightcenter}
    \ce{Zn(s)}$\vert$\ce{ZnSO4(aq)}$\vert\vert$\ce{CuSO4(aq)}$\vert$\ce{Cu(s)}
\end{tightcenter}
...
\begin{theorem}[6A.2.4 电极的种类]
    ...
    \begin{enumerate}[topsep=0pt,parsep=0pt,itemsep=0pt,partopsep=0pt,label=\tbf{(\arabic*)},leftmargin=*]
        \item \tbf{金属/金属离子电极}\\
            由金属浸入含该金属离子的溶液构成,%
            表示为$\ce{M(s)}\vert\ce{M^n+(aq)}$,氧化还原对为\ce{M^n+/M},半反应为\ce{M^n+(aq) + n e^- -> M(s)}.\\
            我们在前面提到的\ce{Zn-Cu}电池的两个电极就属于此类.
        \item \tbf{气体电极}\\
            由气体单质通入含对应离子的溶液构成.由于气体不导电,因此需借助\ce{Pt},石墨等惰性物质进行导电.%
            根据所用气体的不同有不同的表现形式.对于还原性气体,例如\ce{H2-Pt}电极,可以表示为%
            $\ce{Pt(s)}\vert\ce{H2(g)}\vert\ce{H^+(aq)}$,氧化还原对为\ce{H^+/H2},半反应为\ce{2H^+(aq) + 2e^- -> H2(g)}.%
            对于氧化性气体,如\ce{Cl2},\ce{O2}等,书写方法是类似的.\\
            标准氢电极(我们将在介绍标准电极电势时提到)就属于此类.
        \item \tbf{难溶盐电极}\\
            由金属及其表面覆盖一薄层该金属的难溶盐后进入含对应阴离子的溶液构成,%
            可以表示为$\ce{M(s)}\vert\ce{MX(s)}\vert\ce{X^-(aq)}$,氧化还原对为\ce{MX(s)/M(s)},半反应为\ce{MX(s) + e^- -> M(s) + X^-(aq)}.\\
            甘汞电极就属于此类.
        \item \tbf{氧化还原电极}\\
            由惰性导体(如\ce{Pt}等)浸入含某种离子的不同氧化态的溶液中构成.%
            以\ce{Pt-Fe^2+ -Fe^3+}电极为例,可以表示为$\ce{Pt(s)}\vert\ce{Fe^2+(aq)},\ce{Fe^3+(aq)}$,氧化还原对为\ce{Fe^3+(aq)/Fe^2+(aq)},半反应为\ce{Fe^3+(aq) + e^- -> Fe^2+(aq)}.\\
            醌-氢醌电极就属于此类.
    \end{enumerate}
\end{theorem}
